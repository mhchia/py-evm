...
def create_purity_checker_tx(TransactionClass):
    ...
    return create_transaction_from_hex(purity_checker_tx_hex, TransactionClass)
...
def mk_initiating_contracts(sender_privkey,
    ...
    purity_checker_tx = create_purity_checker_tx(TransactionClass)
    ...
    for tx in (purity_checker_tx, viper_rlp_decoder_tx, sighasher_tx, vmc_tx):
...
def deploy_initiating_contracts(vmc_handler, privkey):
    ...
    for tx in txs[:4]:
        ...
        logger.debug(
            "fund_transaction: get_transaction_receipt=%s",
            vmc_handler.web3.eth.getTransactionReceipt(tx_hash)
        )
    mine(vmc_handler, 1)
    for tx in txs[4:]:
        tx_hash = send_raw_transaction(vmc_handler, tx)
        logger.debug(
            "contract transaction: get_transaction_receipt=%s",
            vmc_handler.web3.eth.getTransactionReceipt(tx_hash)
        )
        mine(vmc_handler, 1)
    logger.debug(
